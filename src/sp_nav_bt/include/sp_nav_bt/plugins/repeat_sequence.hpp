 ...
 * ---------------------------------------------------------------------
 *  RepeatSequence     |      Restart          | Tick All Previous Again
 *
 ...
 *                                  - RepeatSequence returns RUNNING and no other nodes are ticked.
 * | SUCCESS | RUNNING |  IDLE   |  - This time A returns SUCCESS so B gets ticked as well
 *                                  - RepeatSequence returns RUNNING and C is not ticked yet
 * | RUNNING | SUCCESS | RUNNING |  - A gets ticked and returns RUNNING, but since it had previously
 *                                  - returned SUCCESS, RepeatSequence continues on and ticks B.
 ...
 *                                  - returned SUCCESS, ending the sequence. RepeatSequence
 *                                  - returns SUCCESS and halts A.
 *
 * If any children at any time had returned FAILURE. RepeatSequence would have returned FAILURE
 * and halted all children, ending the sequence.
 *
 * Usage in XML: <RepeatSequence>
 */
class RepeatSequence : public BT::ControlNode
{
public:
  /**
   * @brief A constructor for nav2_behavior_tree::RepeatSequence
...
#endif  // SP_DECISION_REPEAT_SEQUENCE_NODE
